...
if __name__ == "__main__":
    img_bgr = cv2.imread('./images/arranged.jpg')
    img_rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
    template_bgr = plt.imread('./images/crop.jpg')
    ...
    #plt.show()
    start = time.time()
    points_list = invariant_match_template(rgbimage=img_rgb, rgbtemplate=template_rgb, method="TM_CCOEFF_NORMED", matched_thresh=0.5, rot_range=[0,360], rot_interval=5, scale_range=[100,120], scale_interval=20, rm_redundant=True, minmax=True)
    fig, ax = plt.subplots(1)
    plt.gcf().canvas.manager.set_window_title('Template Matching Results: Rectangles')
    ax.imshow(img_rgb)
    centers_list = []
    elapsed_time = (time.time() - start) * 1000
    print(f"elapsed_time in {elapsed_time:.2f} ms")
